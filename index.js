...
const { spawn } = require('child_process'); // Tambahkan di atas

async function startSSHServer() {
    try {
        console.log('[SSH] Setting up SSH server...');

        // Setup user dan direktori
        await exec(`mkdir -p /run/sshd /var/empty /tmp/home/${SSH_USER}`);
        await exec(`useradd -m -d /tmp/home/${SSH_USER} -s /bin/bash ${SSH_USER} 2>/dev/null || true`);
        await exec(`echo "${SSH_USER}:${SSH_PASS}" | chpasswd 2>/dev/null || true`);

        // Generate host keys
        await exec(`ssh-keygen -A 2>/dev/null || true`);

        // Tulis config
        fs.writeFileSync('/tmp/sshd_config', `
        Port ${SSH_PORT}
        ...
        // START SSH DENGAN SPAWN (biar tetap jalan)
        const sshd = spawn('/usr/sbin/sshd', [
            '-D',
            '-f', '/tmp/sshd_config',
            '-e'
        ], {
        ...
        sshd.stdout.on('data', (data) => {
            console.log(`[SSHD] ${data.toString().trim()}`);
        });

        sshd.stderr.on('data', (data) => {
            console.error(`[SSHD ERR] ${data.toString().trim()}`);
        });

        sshd.on('exit', (code) => {
            console.log(`[SSHD] Exited with code ${code}`);
            // Restart jika mati
            if (code !== 0) {
                console.log('[SSHD] Restarting...');
                setTimeout(() => startSSHServer(), 2000);
            }
        });

        console.log(`[SSH] OpenSSH Server started on port ${SSH_PORT}`);

        // Test dengan telnet (nc tidak tersedia)
        setTimeout(async () => {
            try {
                const test = await exec(`timeout 1 telnet 127.0.0.1 ${SSH_PORT} 2>&1 || true`);
                if (test.stdout.includes('SSH') || test.stderr.includes('SSH')) {
                    console.log('[SSH] ✅ Port test SUCCESS');
                } else {
                    console.log('[SSH] ⚠️ Port test: Maybe running, check logs');
                }
            } catch(e) {
                console.log('[SSH] ⚠️ Port test: Skip (telnet not available)');
            }
        }, 3000);
...
async function startBackgroundServices() {
    ...
    try {
        ...
        // Xray dengan spawn
        const xray = spawn(webPath, ['-c', `${FILE_PATH}/config.json`], {
            stdio: ['ignore', 'pipe', 'pipe'],
            detached: true
        });
        xray.unref();
        ...
        const argo = spawn(botPath, tunnelArgs, {
            stdio: ['ignore', 'pipe', 'pipe'],
            detached: true
        });
        argo.unref();
        ...
        argo.stdout.on('data', (data) => {
            console.log(`[ARGO] ${data.toString().trim()}`);
        });
        argo.stderr.on('data', (data) => {
            console.error(`[ARGO ERR] ${data.toString().trim()}`);
        });
...
class HybridServer {
    ...
    handleSSHWebSocket(ws) {
        console.log('[SSH-WS] New WebSocket connection request');
        ...
        sshSocket.on('connect', () => {
            console.log('[SSH-WS] ✅ Connected to sshd');
        });

        sshSocket.on('error', (err) => {
            console.error('[SSH-WS] SSH socket error:', err.message);
            if (ws.readyState === WS_READY_STATE_OPEN) ws.close(1011);
        });

            // sshd → client: langsung forward semua data tanpa tunggu apapun
            sshSocket.on('data', (chunk) => {
                this.stats.tx += chunk.length;
                if (ws.readyState === WS_READY_STATE_OPEN) {
                    ws.send(chunk, { binary: true }, (err) => {
                        if (err) console.error('[SSH-WS] WS send error:', err.message);
                    });
                }
            });

            // client → sshd: langsung forward, buffer jika socket belum ready
            ws.on('message', (data) => {
                const chunk = Buffer.isBuffer(data) ? data : Buffer.from(data, 'binary');
                this.stats.rx += chunk.length;
                if (sshSocket.writable) {
                    sshSocket.write(chunk, (err) => {
                        if (err) console.error('[SSH-WS] TCP write error:', err.message);
                    });
                }
            });

            sshSocket.on('close', () => {
                console.log('[SSH-WS] sshd closed');
                if (ws.readyState === WS_READY_STATE_OPEN) ws.close();
            });

                ws.on('close', () => {
                    console.log('[SSH-WS] client closed');
                    sshSocket.destroy();
                });

                ws.on('error', (err) => {
                    console.error('[SSH-WS] WS error:', err.message);
                    sshSocket.destroy();
                });
    }
